...
5. Create a Cloud Firestore database. Production mode is recommended.
6. Publish the contents of `firestore.rules` in **Firestore Database → Rules**. These rules permit an authenticated user to access only `users/{their uid}/...`.
...
2. Add all six `NEXT_PUBLIC_FIREBASE_*` values from `.env.example` in **Project → Settings → Environment Variables**.
